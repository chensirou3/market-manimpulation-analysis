"""
Extended Timeframe Backtest - 4H and Daily

Test asymmetric strategy on longer timeframes to capture multi-week/month manipulation cycles.

Timeframes: 4h, 1d (in addition to existing 5min, 15min, 30min, 60min)
"""
...
def load_5min_bars():
    """Load all 5min bars as base data"""
    results_dir = Path('results')
    files = sorted(results_dir.glob('bars_5min_with_manipscore_*.csv'))
...
def build_and_fit_bars(base_bars, bar_size):
    """
    Build bars at target timeframe and fit ManipScore model.
    
    Args:
        base_bars: 5min bars with microstructure features
        bar_size: Target timeframe ('4h', '1d', etc.)
    
    Returns:
        bars: Resampled bars with ManipScore
        model: Fitted ManipScore model
    """
    print(f"  Building {bar_size} bars from 5min data...")
    
    # Resample to target timeframe
    bars = resample_bars_from_lower_tf(base_bars, bar_size)
    print(f"  Built {len(bars):,} {bar_size} bars")
    
    # Fit ManipScore model
    print(f"  Fitting ManipScore model for {bar_size}...")
    model = fit_manipscore_model(bars, bar_size)
    
    # Apply ManipScore
    bars = apply_manipscore(bars, model)
    print(f"  ManipScore fitted and applied")
...
def main():
    print("="*80)
    print("EXTENDED TIMEFRAME BACKTEST - 4H and Daily")
    print("="*80)
    print()
    print("Strategy: Asymmetric (UP=continuation, DOWN=reversal)")
    print("Timeframes: 4h, 1d")
    print()

    # Load base 5min data
    print("Loading 5min base data...")
    base_bars = load_5min_bars()
    print(f"Loaded {len(base_bars):,} 5min bars")
    print(f"Date range: {base_bars.index[0]} to {base_bars.index[-1]}")
    print()
    ...
    for bar_size in bar_sizes:
        ...
        # Build bars and fit ManipScore
        bars, model = build_and_fit_bars(base_bars, bar_size)

        # Save bars for future use
        output_dir = Path('results')
        output_file = output_dir / f'bars_{bar_size}_with_manipscore_full.csv'
    ...
    df_pure.to_csv('results/extended_asymmetric_pure_results.csv', index=False)
    df_managed.to_csv('results/extended_asymmetric_managed_results.csv', index=False)
    ...
    print("\n✅ Saved:")
    print("  - results/extended_asymmetric_pure_results.csv")
    print("  - results/extended_asymmetric_managed_results.csv")
