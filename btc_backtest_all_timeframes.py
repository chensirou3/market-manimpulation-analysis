"""
BTC Backtest - All Timeframes
Test asymmetric strategy on BTC across all timeframes
"""
import pandas as pd
import numpy as np
from pathlib import Path
from dataclasses import dataclass

from src.strategies.trend_features import compute_trend_strength
from src.strategies.extreme_reversal import ExtremeReversalConfig, compute_extreme_trend_thresholds
from src.strategies.backtest_reversal import run_extreme_reversal_backtest


def generate_asymmetric_signals(bars, config):
    """
    Generate asymmetric signals: UP=continuation (LONG), DOWN=reversal (LONG)
    """
    signals = pd.DataFrame(index=bars.index)
    signals['signal'] = 0

    # Compute trend features
    bars = compute_trend_strength(bars, L_past=config.L_past, vol_window=config.vol_window)

    # Compute thresholds
    thresholds = compute_extreme_trend_thresholds(bars, quantile=config.q_extreme_trend)
    threshold = thresholds['threshold']

    # Extreme UP: TS > threshold (positive extreme)
    extreme_up = bars['TS'] > threshold

    # Extreme DOWN: TS < -threshold (negative extreme)
    extreme_down = bars['TS'] < -threshold

    # High ManipScore
    high_manip = bars['ManipScore'] > bars['ManipScore'].quantile(config.q_manip)

    # Asymmetric logic
    # UP + high manip -> LONG (continuation)
    signals.loc[extreme_up & high_manip, 'signal'] = 1

    # DOWN + high manip -> LONG (reversal)
    signals.loc[extreme_down & high_manip, 'signal'] = 1

    # Shift to avoid look-ahead
    signals['signal'] = signals['signal'].shift(1)

    # Fill NaN with 0
    signals['signal'] = signals['signal'].fillna(0)

    return signals


def run_backtest_for_timeframe(timeframe, use_sl_tp=False):
    """Run backtest for a specific timeframe"""
    
    # Load bars
    bars_file = Path(f'results/bars_{timeframe}_btc_with_manipscore_full.csv')
    if not bars_file.exists():
        print(f"  SKIP {timeframe}: file not found")
        return None
    
    bars = pd.read_csv(bars_file, index_col=0, parse_dates=True)
    
    # Config
    config = ExtremeReversalConfig(
        bar_size=timeframe,
        L_past=5,
        vol_window=20,
        q_extreme_trend=0.9,
        q_manip=0.9,
        holding_horizon=5,
        atr_window=10,
        sl_atr_mult=0.5 if use_sl_tp else 999.0,  # Very large value = no SL
        tp_atr_mult=0.8 if use_sl_tp else 999.0,  # Very large value = no TP
        cost_per_trade=0.0001
    )
    
    # Generate signals
    signals = generate_asymmetric_signals(bars, config)

    # Add signals to bars
    bars['exec_signal'] = signals['signal']

    # Run backtest
    result = run_extreme_reversal_backtest(bars, bars['exec_signal'], config, initial_capital=10000)
    
    return result


print("=" * 80)
print("BTC Backtest - All Timeframes")
print("=" * 80)
print()

timeframes = ['5min', '15min', '30min', '60min', '4h', '1d']

results_summary = []

for tf in timeframes:
    print(f"Testing {tf}...")

    try:
        # Pure factor (no SL/TP)
        print(f"  Pure factor...")
        result_pure = run_backtest_for_timeframe(tf, use_sl_tp=False)

        # With SL/TP
        print(f"  With SL/TP...")
        result_sltp = run_backtest_for_timeframe(tf, use_sl_tp=True)
    except Exception as e:
        print(f"  ERROR: {e}")
        result_pure = None
        result_sltp = None

    if result_pure is not None and result_sltp is not None:
        # Debug: print stats keys
        if tf == '5min':
            print(f"  DEBUG: Pure stats keys: {list(result_pure.stats.keys())}")

        summary = {
            'Timeframe': tf,
            'Pure_Sharpe': result_pure.stats.get('sharpe', result_pure.stats.get('sharpe_ratio', 0)),
            'Pure_Return': result_pure.stats.get('total_return_pct', result_pure.stats.get('total_return', 0)),
            'Pure_Trades': result_pure.stats.get('n_trades', len(result_pure.trades)),
            'Pure_WinRate': result_pure.stats.get('win_rate', 0),
            'SLTP_Sharpe': result_sltp.stats.get('sharpe', result_sltp.stats.get('sharpe_ratio', 0)),
            'SLTP_Return': result_sltp.stats.get('total_return_pct', result_sltp.stats.get('total_return', 0)),
            'SLTP_Trades': result_sltp.stats.get('n_trades', len(result_sltp.trades)),
            'SLTP_WinRate': result_sltp.stats.get('win_rate', 0),
        }
        results_summary.append(summary)

        print(f"  Pure: Sharpe={summary['Pure_Sharpe']:.2f}, Return={summary['Pure_Return']:.2f}%, Trades={summary['Pure_Trades']}")
        print(f"  SLTP: Sharpe={summary['SLTP_Sharpe']:.2f}, Return={summary['SLTP_Return']:.2f}%, Trades={summary['SLTP_Trades']}")
    
    print()

# Save results
df_summary = pd.DataFrame(results_summary)
output_file = Path('results/btc_all_timeframes_comparison.csv')
df_summary.to_csv(output_file, index=False)

print("=" * 80)
print("Results Summary")
print("=" * 80)
print(df_summary.to_string(index=False))
print()
print(f"Saved to {output_file}")

